    ...
    function reply($cron = 2, $replyFile = "data.txt", $replyPatternFile = "reply_pattern.php"){
        ...
        if(count($replies) != 0){                           
            ...
            foreach($replies as $rep){
				...
				// 20160509
				// ユーザー名の「@」を「*」に置換する
				$rep["user"]["name"] = str_replace("@", "*", $rep["user"]["name"]);
        ...
        if($followlist){
            foreach($followlist as $id){    
                ...
                if(empty($response["errors"])){
                    ...
                    $message = $date;
                    $message .= "   ";
                    $message .= $response["name"]."(@<a href='https://twitter.com/".$response["screen_name"]."'>".$response["screen_name"]."</a>)をフォローしました";
                    echo $message;
                    echo "\n";
                    $results = $message;        
                }
...
//============================================================
    ...
    function makeReplyTweets($replies, $replyFile, $replyPatternFile){
        ...
        foreach($replies as $reply){
			...
			// 会話数制限（リプライ返し）
			$filenameb = "log/kaiwalog.txt";//kaiwalog.txtに誰からリプライがあったか最新$botk1件記録する
			//$filenamebは存在するか？
			if(!file_exists($filenameb)){
				touch($twit_logfile) or die('ファイル作成に失敗\n');
				chmod($twit_logfile, 0666) or die('権限変更に失敗\n');//※パーミッションは鯖によって違います
			}
			$botdata = file_get_contents($filenameb);
//			$countk = substr_count($botdata,(string)$reply->user->screen_name);
			$countk = substr_count($botdata,(string)$reply["user"]["screen_name"]);
...
//			if(preg_match("@bot@i",(string)$reply->user->screen_name) || in_array((string)$reply->user->screen_name,$bottername)){
			if(preg_match("@bot@i",(string)$reply["user"]["screen_name"]) || in_array((string)$reply["user"]["screen_name"],$bottername)){
				if ($countk>=$botk2){
					...
					while ($kazu>$botk1){
						$botdatax = array_pop($botdata);$kazu = count($botdata);
					}
					$botdata=implode(",",$botdata);
    ...
    function makeReplyTimelineTweets($timeline, $replyPatternFile){
        ...
        foreach($timeline as $tweet){
			// 20160509
			// ユーザー名の「@」を「*」に置換する
			$tweet["user"]["name"] = str_replace("@", "*", $tweet["user"]["name"]);
